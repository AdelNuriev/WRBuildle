...
public class ImposterHandler extends BaseHandler implements ChallengeHandler {
    private final ChallengeService challengeService;
    private final ObjectMapper objectMapper;

    public ImposterHandler(ChallengeService challengeService) {
    ...
    public void handle(HttpServletRequest req, HttpServletResponse resp, User user)
        ...
        ChallengeBlock challenge = challengeService.getChallengeBlock(LocalDate.now(), BlockType.IMPOSTER);
        UserResult userResult = challengeService.getUserResult(user.getId(), LocalDate.now(), BlockType.IMPOSTER);
